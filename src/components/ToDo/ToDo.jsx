...
function Input(props) {
  ...
  return (
    <form
      onSubmit={e => {
        e.preventDefault();
        props.setTodos(prevTodos => [...prevTodos, { id: Date.now(), text: todo }]);
        setTodo("");
      }}
    >
      <input type="text" className="input" value={todo} onChange={e => setTodo(e.target.value)} placeholder="Enter text!" />
...
function TodoItem(props) {
  ...
  return (
    <div className={["sub-todo-item", props.todo.done && "done"].filter(Boolean).join(" ")}>
      {isEdit ? <EditInput setTodos={props.setTodos} todo={props.todo} setIsEdit={setIsEdit} /> : props.todo.text}
...
function ToDo() {
  ...
  return (
    <div className="ToDo">
      {/* <img className="title" src="logo192.jpg" height="20" width="60" alt="logo" /> */}
          ...
          <div>
            <li key={todo.id} className="todo-item">
              ...
              <button
                ...
                onClick={() => setTodos(prevTodos => prevTodos.filter(item => item.id !== todo.id))}
              >
                Delete
              </button>
            </li>
